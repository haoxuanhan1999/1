...
class MTLEgoNuScenesDataset(NuScenesDataset):
    ...
    def get_temporal_indices(self, index):  #返回当前帧，历史帧的索引
    ...
    @staticmethod
    def get_egopose_from_info(info):  #返回自车到全局变换矩阵
    ...
    def get_data_info(self, index):  #获取指定索引的数据信息
        ...
        for prev_index in prev_indices:
            if prev_index >= 0:
                img_infos.append(self.data_infos[prev_index]['cams'])  #获取历史帧的图像信息
                lidar_infos.append(self.data_infos[prev_index]['lidar_path'])      #添加点云信息
            else:
                # get the information of current frame for invalid frames
                img_infos.append(info['cams'])
                lidar_infos.append(info['lidar_path'])
        ...
        input_dict['lidar_info'] = lidar_infos
