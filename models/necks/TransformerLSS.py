...
class TransformerLSS(BaseModule):
    ...
    def forward(self, input, flip_x=False, flip_y=False):
        x, rots, trans, intrins, post_rots, post_trans = input  #添加外参后需修改
        B, S, N, C, H, W = x.shape
        # flatten (batch, seq, num_cam)
        x = x.view(B * S * N, C, H, W)
        x = self.depthnet(x)

        depth = self.get_depth_dist(x[:, :self.D])  #通过depthnet得到前41个通道表示每个点在4-45米每个深度的特征值
        # [B * S, N, D, H, W, 3]
        geom = self.get_geometry(rots, trans, intrins, post_rots, post_trans)
        cvt_feature_list = [x[:, self.D:(self.D + self.numC_Trans)]]  #后64个通道存放当前点的特征
        ...
        volume = depth.unsqueeze(1) * cvt_feature.unsqueeze(2)  #通道拆分做内积得到立方体特征
